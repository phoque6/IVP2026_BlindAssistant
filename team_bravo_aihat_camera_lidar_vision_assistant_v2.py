#!/usr/bin/env python3
"""
Team Bravo AI HAT Camera + LiDAR Vision Assistant v2 for Blind Navigation Support
================================================================================

v2 improves voice feedback: confirmation counts, repeat intervals, and priority
routing so sign/OCR, camera objects, and LiDAR obstacles are spoken separately
...
Run:
    python3 team_bravo_aihat_camera_lidar_vision_assistant_v2.py
...
def process_voice_alerts() -> str:
    ...
    update_sign_voice_state_machine(ocr_text)
...
def run_ocr_on_signs(frame_bgr: np.ndarray, detections: List[Detection]) -> List[OCRResult]:
    ...
    for (x1, y1, x2, y2) in rois[:2]:
        ...
        if OCR_MIN_TEXT_LENGTH <= len(cleaned) <= OCR_MAX_TEXT_LENGTH:
...
def main() -> None:
    ...
    pygame.init()
    pygame.display.set_caption("Team Bravo AI HAT Camera + LiDAR Vision Assistant v2")
