#!/usr/bin/env python3
"""
Team Bravo Vision Assistant v12 — v11 OCR/YOLO + v8 voice feedback
==================================================================================

v12 = early v11 camera fixes + v8 voice feedback timing/priority/phrases:
  - OCR: numbers + longer road signs (from v11)
  - YOLO: name objects and voice when ≤ 5 metres (from v11)
  - Voice: v8 confirmation counts, repeat intervals, interrupt priority, phrases
  - LiDAR obstacle safety + beginner SLAM floor-plan map kept
...
Run:
    python3 team_bravo_aihat_camera_lidar_vision_assistant_v12_v8_voice_ocr_yolo.py
...
# Signs: v8 voice repeat; OCR still scans fairly often for numbers/streets
...
# Sign / OCR voice — v8: require 2 stable reads before speaking
SIGN_CONFIRM_DETECTIONS = 2
SIGN_REPEAT_SECONDS = 5.0

# Camera YOLO object voice — v8 confirm/repeat + v11 ≤5 m gate
...
# LiDAR obstacle voice — v8 confirmation + repeat intervals
...
# Path clear voice — v8: sustained clear before resetting obstacle alert
...
# OCR debug + voting (v11 sign-aware, any readable text)
ocr_debug_raw = ""
ocr_debug_cleaned = ""
ocr_debug_matched = ""
ocr_debug_confirmed = ""
...
def test_voice_stop() -> None:
    stop_current_voice()
    run_tts("Stop. Obstacle very close behind you", force=True)
...
def print_voice_settings() -> None:
    print("Voice settings (v8 feedback + v11 OCR/YOLO):")
    print(f"- Sign confirm detections: {SIGN_CONFIRM_DETECTIONS}")
    print(f"- Sign / OCR voice repeat: {OCR_VOICE_REPEAT_SECONDS:.0f} seconds")
...
def update_lidar_voice_state_machine(raw_alert: str) -> None:
    """Confirm obstacles after several scans; only clear after sustained CLEAR (v8)."""
...
def build_lidar_speech(lidar_alert: str, object_label: Optional[str] = None) -> str:
    """v8 LiDAR obstacle phrases — clear stop / careful / direction guidance."""
    ...
    if lidar_alert.startswith("STRONG_"):
        ...
        if direction == "behind":
            return "Careful. Obstacle behind you"
...
def build_sign_speech(text: str) -> str:
    """v8-style sign reading — also expands Ave/St/Rd and numbers from v11 OCR."""
...
def build_object_speech(label: str, direction: str) -> str:
    """v8 camera object phrases (used when path clear and object ≤ 5 m)."""
...
def main() -> None:
    ...
    pygame.init()
    pygame.display.set_caption("Team Bravo Vision Assistant v12 — v8 voice + OCR/YOLO")
    screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.RESIZABLE)
    print("Pi 5 v12: v8 voice feedback | OCR numbers/road signs | YOLO objects ≤5 m | Mute 30s / N")
