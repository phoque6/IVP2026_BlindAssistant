#!/usr/bin/env python3
"""
Team Bravo AI HAT Camera + LiDAR Vision Assistant v4 for Blind Navigation Support
================================================================================

v4 fixes voice (Windows/Linux TTS), adds on-screen voice test buttons, and improves
camera frame rate by decoupling capture from heavy AI/OCR work.
...
Run:
    python3 team_bravo_aihat_camera_lidar_vision_assistant_v4.py
...
# Sign / OCR voice
SIGN_CONFIRM_DETECTIONS = 10
SIGN_REPEAT_SECONDS = 10.0
...
def choose_voice_message(
    ...
    Priority: VERY_CLOSE > STRONG > sign > lidar normal > camera object > path clear.
    """
    now = time.time()
    ...
    if confirmed_sign_text and not confirmed_lidar_alert.startswith(("VERY_CLOSE_", "STRONG_")):
        if (
            confirmed_sign_text != last_spoken_sign_text
            or (now - last_sign_voice_time) >= SIGN_REPEAT_SECONDS
        ):
...
def init_ocr_system() -> bool:
    ...
    try:
        ...
        return False
    print(
        f"OCR scans every {OCR_EVERY_N_FRAMES} camera frames; "
        f"voice speaks sign after {SIGN_CONFIRM_DETECTIONS} stable reads"
    )
    return True
...
def _ocr_single_roi(gray: np.ndarray, bbox: Tuple[int, int, int, int], results: List[OCRResult]) -> None:
    ...
    for psm in (7, 8, 6):
        try:
            txt = pytesseract.image_to_string(binary, config=f"--oem 3 --psm {psm}")
        except Exception:
            continue
...
def run_ocr_on_signs(frame_bgr: np.ndarray, detections: List[Detection]) -> List[OCRResult]:
    ...
    rois.append((0, 0, w, max(60, int(h * 0.38))))
    if not any(s in d.label.lower() for s in sign_like for d in detections):
        rois.append((0, 0, w, h))
...
def apply_ocr_scan_results(new_items: List[OCRResult]) -> None:
    """Persist OCR on camera thread; count sign confirmations per OCR scan."""
    ...
    if new_items:
        ...
        for item in new_items:
            ...
            update_sign_voice_state_machine(item.text)
    else:
        with camera_lock:
            stale = (now - last_ocr_update_time) > OCR_PERSIST_SECONDS if last_ocr_update_time else True
            if stale and latest_ocr_results:
                latest_ocr_results = []
...
def run_camera_detections(frame_bgr: np.ndarray, frame_n: int) -> List[Detection]:
    """Fast-tier detection on a downscaled frame; empty list means keep previous dets."""
    if cv2 is None:
        return []
    if frame_n % max(1, AI_INFERENCE_EVERY_N_FRAMES) != 0:
        return []
    h, w = frame_bgr.shape[:2]
    small = cv2.resize(
...
def camera_thread_fn() -> None:
    ...
    global last_camera_banner, camera_frame_counter, last_ocr_text
    ...
    while running:
        ...
        if frame_bgr is None:
            ...
            apply_ocr_scan_results(ocr_items)
        ...
        with camera_lock:
            ...
            # latest_ocr_results updated only in apply_ocr_scan_results()
...
def draw_ai_camera_panel(
    ...
    frame_for_draw = frame_snapshot
    if frame_snapshot is not None and ai_overlay_enabled and cv2 is not None:
        bgr = cv2.cvtColor(frame_snapshot, cv2.COLOR_RGB2BGR)
        bgr = draw_ai_detections(bgr, detections_snapshot)
        for o in ocr_snapshot:
            x1, y1, x2, y2 = o.bbox
            cv2.rectangle(bgr, (x1, y1), (x2, y2), (255, 220, 40), 2)
            cv2.putText(
                bgr, f"Text: {o.text}", (x1, min(bgr.shape[0] - 10, y2 + 18)),
                cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 220, 40), 2, cv2.LINE_AA,
            )
        frame_for_draw = cv2.cvtColor(bgr, cv2.COLOR_BGR2RGB)
    if frame_for_draw is not None:
        surf = pygame.surfarray.make_surface(frame_for_draw.swapaxes(0, 1))
...
def draw_footer(screen: pygame.Surface) -> None:
    ...
    controls = (
        "Q Quit | F Fullscreen | 0 Quad | 1-4 Panels | Z Zones | T Voice test | V Voice | "
        "C Cam | S Save | O OCR | U Fusion | D Debug"
    )
...
def draw_debug_panel(
    ...
    lines = [
        ...
        f"model={AI_MODEL_PATH} camera_fps={camera_fps:.1f} ai_fps={ai_inference_fps:.1f}",
...
def main() -> None:
    ...
    pygame.init()
    pygame.display.set_caption("Team Bravo AI HAT Camera + LiDAR Vision Assistant v4")
    screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.RESIZABLE)
    print("View: F=fullscreen | 0=quad | 1-4=panels | click panel title or footer voice buttons")
    print("Pi 5: camera panel shows 'Sign: EXIT' when OCR reads text; press D for debug counts")
