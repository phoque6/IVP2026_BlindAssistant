#!/usr/bin/env python3
"""
Team Bravo AI HAT Vision Assistant v5 restore v3 camera
========================================================

v5 restore: robust USB index scan (0-5), PREFER_USB_CAMERA priority, PiCam fallback.
...
Run:
    python3 team_bravo_aihat_camera_lidar_vision_assistant_v5_restore_v3_camera.py
...
def init_usb_camera() -> bool:
    ...
    global active_usb_camera_index, using_explicit_simulation
    ...
    for idx in USB_CAMERA_INDEXES:
        ...
        if cap is not None:
            ...
            camera_error_message = ""
            using_explicit_simulation = False
            print(f"Camera OK: USB camera index {idx}")
            return True
...
def release_camera() -> None:
    ...
    global _picam_instance, _usb_cap, active_usb_camera_index
...
def draw_ai_camera_panel(
    ...
    screen.blit(
        font.render(
            f"Source:{camera_display_name()}  {camera_fps:.0f}fps  AI:{ai_inference_fps:.0f}fps  dets:{len(detections_snapshot)}",
            True, COLOR_CYAN,
        ),
        (rect.x + 6, y),
    )
...
def draw_debug_panel(
    ...
    lines = [
        ...
        f"active_usb_camera_index={active_usb_camera_index}",
...
def main() -> None:
    ...
    pygame.init()
    pygame.display.set_caption("Team Bravo Vision Assistant v5 restore v3 camera")
    screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.RESIZABLE)
    print("Pi 5: C=retry camera | P=USB/PiCam priority | 1=camera fullscreen | D=debug")
