#!/usr/bin/env python3
"""
Team Bravo Vision Assistant v6 — USB fast capture, OCR thread, AI HAT status
=============================================================================

v6: 3-thread camera (capture / AI / OCR), MJPEG USB scan, freeze watchdog, honest AI HAT status.
...
Run:
    python3 team_bravo_aihat_camera_lidar_vision_assistant_v6_usb_fast_ocr_aihat.py
...
def print_camera_settings() -> None:
    ...
    print("Camera: C=retry | X=on/off | P=USB/PiCam priority")
...
def clean_ocr_text(text: str) -> str:
    ...
    corrections = {
        "EX1T": "EXIT", "EX1 T": "EXIT", "EX1": "EXIT", "E X I T": "EXIT",
    ...
    if len(compact) >= 4 and compact.count(compact[0]) / len(compact) > 0.85:
        return ""
    # Reject mostly non-alphanumeric noise
...
def run_ocr_on_signs(frame_bgr: np.ndarray, detections: List[Detection]) -> List[OCRResult]:
    ...
    cx1, cy1 = int(w * 0.12), int(h * 0.18)
    cx2, cy2 = int(w * 0.88), int(h * 0.82)
    rois.append((cx1, cy1, cx2, cy2))
...
def apply_ocr_scan_results(new_items: List[OCRResult]) -> None:
    """Persist OCR results and update sign voice state; keep last text for OCR_PERSIST_SECONDS."""
    ...
    if new_items:
        ...
        print(f"OCR FOUND: {best.text}")
        update_sign_voice_state_machine(best.text)
    elif last_ocr_persist_until > 0 and now > last_ocr_persist_until:
        with camera_lock:
...
def camera_ocr_thread_fn() -> None:
    ...
    while running:
        ...
        apply_ocr_scan_results(run_ocr_on_signs(frame, dets))
...
def draw_ai_camera_panel(
    ...
    if focused_panel == 1:
        ...
        for ln in (
            ...
            "C=retry  X=cam on/off  P=USB/PiCam priority",
        ):
...
def draw_footer(screen: pygame.Surface) -> None:
    ...
    controls = (
        ...
        "C Retry Camera | X Cam On/Off | P USB/PiCam Priority | "
...
def main() -> None:
    ...
    pygame.init()
    pygame.display.set_caption("Team Bravo Vision Assistant v6 USB fast OCR AI HAT")
    screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.RESIZABLE)
    print("Pi 5 v6: 3-thread camera | C=retry | P=USB/PiCam | 1=camera fullscreen | D=debug")
