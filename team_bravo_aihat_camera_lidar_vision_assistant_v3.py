#!/usr/bin/env python3
"""
Team Bravo AI HAT Camera + LiDAR Vision Assistant v3 for Blind Navigation Support
================================================================================

v3 fixes OCR sign reading (persist results between scans), restores view controls
(fullscreen, single-panel focus, zones overlay), and keeps v2 voice anti-spam logic.
...
Run:
    python3 team_bravo_aihat_camera_lidar_vision_assistant_v3.py
...
def speak_chosen_message(
    ...
    if not voice_enabled:
        return False
    if not interrupt and category != "lidar_very_close" and is_voice_speaking():
        return False
    if interrupt or (category == "lidar_very_close" and is_voice_speaking()):
        stop_current_voice()
...
def draw_footer(screen: pygame.Surface) -> None:
    ...
    controls = (
        "Q Quit | F Fullscreen | 0 Quad | 1 Cam | 2 Zones | 3 LiDAR | 4 Map | Z Zones overlay | "
        "C Camera | S Save | V Voice | O OCR | U Fusion | D Debug | Space Pause | +/- Zoom"
    )
    font = pygame.font.SysFont("monospace", 12)
...
def main() -> None:
    ...
    pygame.init()
    pygame.display.set_caption("Team Bravo AI HAT Camera + LiDAR Vision Assistant v3")
    screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.RESIZABLE)
    print("View controls: F=fullscreen | 0=quad | 1-4=panel focus | Z=zones overlay | click panel title")
    if pytesseract is None:
        print("WARNING: pytesseract not installed — OCR sign reading disabled.")
    elif not shutil.which("tesseract"):
        print("WARNING: tesseract binary not found — install tesseract-ocr for sign reading.")
