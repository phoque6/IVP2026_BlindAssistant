#!/usr/bin/env python3
"""
Team Bravo Vision Assistant v9 — SLAM-style LiDAR view, faster sign OCR
==================================================================================

v9: Same as v8 with faster centre-first OCR, sign voice priority, 10s obstacle voice.
...
Run:
    python3 team_bravo_aihat_camera_lidar_vision_assistant_v9_slam_lidar_sign_ocr.py
...
def should_apply_vocab_correction(text: str) -> bool:
    """Only fuzzy-correct short sign-like phrases — not book titles or long text."""
    if not text:
        return False
    words = text.split()
...
def pick_ocr_final_text(cleaned: str, matched: str) -> str:
    """Prefer real OCR text when vocabulary would wrongly replace longer phrases."""
    ...
    # Do not replace multi-word / long OCR (e.g. book title) with one vocabulary word
...
def _ocr_single_roi(
    ...
    variants = _prepare_ocr_variants(roi)
    seen: set = set()
    for img in variants:
        for cfg in _ocr_tesseract_configs(fast=fast):
            try:
...
def find_color_sign_rois(frame_bgr: np.ndarray) -> List[Tuple[int, int, int, int]]:
    ...
    return rois[:OCR_MAX_EXTRA_ROIS]
...
def main() -> None:
    ...
    pygame.init()
    pygame.display.set_caption("Team Bravo Vision Assistant v9 SLAM LiDAR + sign OCR")
    screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.RESIZABLE)
    print("Pi 5 v9: SLAM LiDAR | sign voice priority | obstacle voice 10s | R=read sign | D=debug")
